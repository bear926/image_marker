  ...
  'use strict';
    ...
    $.extend( Plugin.prototype, {
      ...
      mountTo: function($target, $text_box, $dot, $line, onDrag, marker) {
        ...
        if (!this.settings.close_disabled && !this.settings.answer_mode) {
        ...
        var current_marker_drag = marker.drag_disabled;
        if (marker.drag_disabled === 'undefined') {
          current_marker_drag = this.settings.drag_disabled;
        }
        $dot.draggable({
          containment: 'parent',
          distance: 0,
          delay: 0,
          disabled: current_marker_drag
        }, {
          drag: onDrag
        });

        this.$element.on('drag_all', onDrag);
        if (!this.settings.drag_disabled && !this.settings.answer_mode) $text_box.draggable({
          revert: true
        }, {
          start: function() {
